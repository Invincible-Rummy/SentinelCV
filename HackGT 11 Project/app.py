    ...
    def generate():
        while True:
            if patient_id in patient_joint_alerts:
                # Yield the patient joint alert data as JSON
                yield f"data: {patient_joint_alerts}\n\n"
            else:
                yield f""
...
def get_patient_position_alert(patient_id):
    # Gets patient position alert boolean
    def generate():
        while True:
            if patient_id in patient_position_alerts:
                # Yield the patient position alert data as JSON
                yield f"data: {patient_position_alerts}\n\n"
            else:
                yield f""
    return Response(generate(), mimetype='text/event-stream')
