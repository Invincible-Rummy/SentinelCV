...
def draw_bounds(img):
    for patient in patients:
        ...
        if patient.nurse_alert == True:
            cv2.rectangle(img, (patient.x1, patient.y1), (patient.x2, patient.y2), (0, 0, 255), 3)
            cv2.putText(img, "Patient " + str(patient.id) + ": NEEDS TO CHANGE POSTURE", org, font, fontScale, (0, 0, 255), thickness)
        else:
            cv2.rectangle(img, (patient.x1, patient.y1), (patient.x2, patient.y2), (0, 255, 0), 3)
            cv2.putText(img, "Patient " + str(patient.id) + ": OK", org, font, fontScale, (0, 255, 0), thickness)
    ...
    while True:
        ...
        for patient in patients:
            ...
            if landmarks:
                ...
                # Display detected posture on the video feed or alert to change position
                if patient.nurse_alert == True:
                    cv2.putText(img_rgb, "Patient " + str(patient.id) + ": NEEDS TO CHANGE POSTURE", 
